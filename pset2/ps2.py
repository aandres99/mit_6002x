...
class RectangularRoom(object):
    ...
    def __init__(self, width, height):
        ...
        assert height > 0
        self.height = height
        # numpy not implemented in the grader
        # self.tiles = np.full([self.width, self.height], False)
        # so use dictionary with tuple keys
        keys = []
        values = []
        for y in range(self.height):
            for x in range(self.width):
                keys.append((x, y))
        values = [False for _ in range(self.width * self.height)]
        self.tiles = dict(zip(keys, values))
...
class StandardRobot(Robot):
    ...
    def updatePositionAndClean(self):
        ...
        been cleaned.
        """
        new_pos = self.pos.getNewPosition(self.getRobotDirection(), self.speed)
...
# *** RUN IN GRAPHIC CONSOLE like jupyter qtconsole *** #
...
def runSimulation(num_robots, speed, width, height, min_coverage, num_trials,
    ...
    robot_type: class of robot to be instantiated (e.g. StandardRobot or
                RandomWalkRobot)
    """
    assert isinstance(num_robots, int) & (num_robots > 0)
    assert isinstance(speed, float)  & (speed > 0)
    assert isinstance(width, int) & (width > 0)
    assert isinstance(height, int) & (height > 0)
